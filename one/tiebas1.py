...
class GetTie:
    def __init__(self, keywords, count):
        self.keywords = keywords
        self.count = count
        self.url_list = []

        for i in range(self.count):
            self.url_list.append('https://tieba.baidu.com/f?kw={}&pn={}'.format(self.keywords, i * 50))
        # 定义请求头
        self.headers = {
    ...
    def save_data(self, data, num):
        with open('tieba_{}_{}.html'.format(self.keywords, num), 'w', encoding='utf-8') as f:
            f.write(data)

    def start(self):
        for num, url in enumerate(self.url_list):
            data = self.get_data(url)
            self.save_data(data, num + 1)
